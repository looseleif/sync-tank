...
# -*- coding: utf-8 -*-
"""
Unified ROV Dashboard (decoupled control loop):
...
- NEW: MotorController thread runs at fixed rate so RTSP work can't starve PWM.
- NEW: Autonomous "circle" explorer mode (toggle via /auto/start and /auto/stop)
"""
...
class MotorController(threading.Thread):
    ...
    def _compute_auto_targets(self, now: float):
        ...
        tx = mx0 + int(cfg.radius_xy * math.cos(theta))
        ty = my0 + int(cfg.radius_xy * math.sin(theta))
        tz = mz0 + int(cfg.radius_z  * math.sin(zeta))
