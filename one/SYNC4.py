...
# -*- coding: utf-8 -*-
"""
Unified ROV Dashboard (decoupled control loop + axis toggles + ultra-light RTSP + CSI cams):
- Primary: USB cam (always on) + motor controls (PCA9685 over I2C)
- Secondary: RTSP cams (now snapshot w/ throttling, low CPU priority, serialized grabs)
- NEW: CSI camera snapshot workers (2 Pi cams supported; uses libcamera-jpeg fallback)
- MotorController thread runs at fixed rate so camera work can't starve PWM.
...
# UI refresh cadence for IP cards (also the target RTSP snapshot cadence)
CAM_SWAP_SEC = float(os.environ.get("CAM_SWAP_SEC", "5.0"))   # default 5s

# RTSP snapshot cadence + throttles
RTSP_SNAPSHOT_SEC = float(os.environ.get("RTSP_SNAPSHOT_SEC", "5.0"))
RTSP_MAX_PAR = int(os.environ.get("RTSP_MAX_PAR", "1"))       # serialize grabs to 1 to avoid CPU spikes

# CSI snapshot cadence
CSI_SNAPSHOT_SEC = float(os.environ.get("CSI_SNAPSHOT_SEC", "1.0"))
# CSI camera indexes (as seen by libcamera) e.g. "0,1"
...
# Force OpenCV/FFmpeg to use TCP for RTSP and short timeouts
        ...
        for k, v in kw.items():
            if hasattr(self.auto_cfg, k):
                if k in ("period_s", "z_period_s", "tick_hz"):
                    ...
                    # accept true/false/"true"/"1"/1
                    if isinstance(v, str):
                        setattr(self.auto_cfg, k, v.lower() in ("1","true","yes","on"))
                    else:
...
# ------------------ RTSP CAMS (ultra-light snapshot workers) ------------------
rtsp_sem = threading.Semaphore(max(1, RTSP_MAX_PAR))

class CamWorker(threading.Thread):
    """
    Very light-touch RTSP worker:
    - At most RTSP_MAX_PAR workers grab concurrently (global semaphore).
    - Each cycle: try acquire semaphore non-blocking; if busy, skip this cycle.
    - Once acquired: open, grab ONE frame, encode JPEG, release immediately.
    - Lower CPU priority with os.nice(+10) best-effort.
    """
    ...
    def _grab_one_frame(self) -> Optional[bytes]:
        ...
        try:
            ...
            if FRAME_FPS: cap.set(cv2.CAP_PROP_FPS, 5)  # low fps hint
    ...
    def run(self):
        try:
            os.nice(10)  # de-prioritize this thread if permitted
        except Exception:
            pass
        print(f"[CAM{self.idx}] RTSP snapshot worker for {self.url}", flush=True)
        while not self._stop.is_set():
            cycle_start = time.time()
            got = rtsp_sem.acquire(blocking=False)
            if got:
                try:
                    ...
                    if jpg:
                        ...
                        # keep last frame; just log once in a while
                        pass
                finally:
                    rtsp_sem.release()
            # sleep to next cadence without blocking the control loop
...
# ------------------ CSI CAMS (libcamera-jpeg snapshot workers) ------------------
LIBCAMERA_JPEG = shutil.which("libcamera-jpeg") or shutil.which("libcamera-still")  # fallback for some images
LIBCAMERA_TIMEOUT_S = float(os.environ.get("LIBCAMERA_TIMEOUT_S", "6.0"))  # first grab can be slow

class CSICamWorker(threading.Thread):
    """
    Snapshot from CSI camera via libcamera-jpeg (preferred) or OpenCV V4L2 if available.
    Updates an MJPEG buffer with most recent still; runs on its own cadence.
    """
    def __init__(self, idx: int, cam_id: str, title: Optional[str] = None):
        super().__init__(daemon=True)
        self.idx = idx
        self.cam_id = cam_id  # libcamera index as string, e.g., "0"
        self.title = title or f"CSI {cam_id}"
        self.buf = FrameBuffer()
        self._stop = threading.Event()
    ...
    def _grab_with_libcamera(self) -> Optional[bytes]:
        if not LIBCAMERA_JPEG:
            return None
        cmd = os.path.basename(LIBCAMERA_JPEG)
        args = [
            LIBCAMERA_JPEG,
        ...
        try:
            p = subprocess.run(args, timeout=LIBCAMERA_TIMEOUT_S, capture_output=True)
            if p.returncode == 0 and p.stdout:
                return bytes(p.stdout)
            else:
                err = (p.stderr or b"").decode("utf-8", "ignore").strip()
                print(f"[CSI{self.idx}] {cmd} failed (rc={p.returncode}): {err[:200]}", flush=True)
        except subprocess.TimeoutExpired:
            print(f"[CSI{self.idx}] {cmd} timed out after {LIBCAMERA_TIMEOUT_S}s", flush=True)
        except Exception as e:
            print(f"[CSI{self.idx}] {cmd} error: {e}", flush=True)
        return None

    def _grab_with_v4l2(self) -> Optional[bytes]:
        # Try multiple by-path patterns (Pi 5 names vary), then fall back to /dev/videoN
        csi_nodes = sorted(glob.glob("/dev/v4l/by-path/*csi*-video-index0")) \
                or sorted(glob.glob("/dev/v4l/by-path/*video-index0"))
        nodes = [os.path.realpath(n) for n in csi_nodes]
        if not nodes:
            nodes = [p for p in glob.glob("/dev/video[0-9]") if os.path.exists(p)]
        if not nodes:
            return None
    ...
    def run(self):
        ...
        while not self._stop.is_set():
            ...
            # cadence
            elapsed = time.time() - start
            to_sleep = max(0.1, CSI_SNAPSHOT_SEC - elapsed)
            for _ in range(int(to_sleep * 10)):
                if self._stop.is_set(): break
                time.sleep(0.1)
...
class CSIRegistry:
    ...
    def add(self, cam_id: str, title: Optional[str] = None) -> int:
        with self._lock:
            idx = self._next_idx
            self._next_idx += 1
            worker = CSICamWorker(idx, cam_id, title or f"CSI {cam_id}")
            self._cams[idx] = worker
            worker.start()
    ...
    def list(self) -> List[Tuple[int, str, str]]:
        with self._lock:
            return [(i, c.cam_id, c.title) for i, c in sorted(self._cams.items())]
...
# UI carousel index (client refresh hints)
_carousel_idx = 0
...
<head>
  <meta charset="utf-8">
  <title>SYNC TANK - SEE SEA TELEVISION</title>
...
<body>
  <header>
    <strong>SYNC TANK - SEE SEA TELEVISION</strong>
  ...
  <main>
    <section class="hero">
      ...
      <small>
        ...
        RTSP snapshots every {{swap}}s (throttled).
      </small>
      <pre id="status"></pre>
    </section>
...
function renderCSICards(){
  const grid = document.getElementById('csigrid');
  grid.innerHTML = '';
  csis.forEach((c)=>{
    const card = document.createElement('div');
    card.className = 'card';
...
// Periodically bounce one RTSP <img> to pick newest snapshot.
let cur = 0;
setInterval(()=>{
...
// Periodically bounce all CSI images (cheap local grabs)
setInterval(()=>{
  csis.forEach(c=>{
...
def debug_csi():
    return jsonify({
        "libcamera_cmd": LIBCAMERA_JPEG,
        "libcamera_timeout_s": LIBCAMERA_TIMEOUT_S,
        "csi_indexes": CSI_INDEXES,
    })
...
def main():
    ...
    print(f"[READY] http://{HOST}:{PORT}  (USB at /usb, RTSP at /cam/<idx>, CSI at /csi/<idx>)", flush=True)
