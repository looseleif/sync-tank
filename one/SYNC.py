...
# -*- coding: utf-8 -*-
"""
Unified ROV Dashboard:
- Primary: USB cam (always on) + motor controls (PCA9685 over I2C)
- Secondary: RTSP cams (carousel underneath, cycles every CAM_SWAP_SEC)
- Health endpoints + optional hunter to discover a 3rd RTSP
"""

import os, sys, time, signal, subprocess, glob
import threading
...
# Carousel timing for IP cams
CAM_SWAP_SEC = float(os.environ.get("CAM_SWAP_SEC", "0.5"))
...
# Force OpenCV/FFmpeg to use TCP for RTSP unless overridden
os.environ.setdefault("OPENCV_FFMPEG_CAPTURE_OPTIONS", "rtsp_transport;tcp")
FFMPEG_OPEN_FLAGS = "rtsp_transport;tcp|fflags;nobuffer|max_delay;500000|rw_timeout;2000000|stimeout;2000000"
...
def pwm_watchdog():
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            try:
                # re-init
                addr, bus_id = pwm.addr, getattr(pwm.bus, "fd", None)
                _ = PWM(addr=addr)  # new object
                globals()["pwm"] = _
        ...
        while not self._stop.is_set():
            ...
            while not self._stop.is_set():
                ...
                if not ok or frame is None:
                    ...
                    time.sleep(1)
                    break
                # Resize if wildly off (avoid extra CPU)
                if FRAME_W and FRAME_H:
                    h, w = frame.shape[:2]
                    if abs(w - FRAME_W) > 8 or abs(h - FRAME_H) > 8:
                        frame = cv2.resize(frame, (FRAME_W, FRAME_H), interpolation=cv2.INTER_AREA)
    ...
    def add(self, url: str, title: Optional[str] = None) -> int:
        with self._lock:
            if any(c.url == url for c in self._cams.values()):
                # already present, return extant index
...
# Carousel state for IP cams (just for UI hint; switching is done client-side)
_carousel_idx = 0
...
<head>
  ...
  <style>
    ...
    .card .sub { color:#9aa; font-size:12px; padding:0 12px 8px; }
...
<body>
  ...
  <main>
    <section class="hero">
      ...
      <small>Keys: W/A/S/D/Q/E, Space or X = Stop.  Health: <a href="/health" target="_blank">/health</a></small>
      <pre id="status"></pre>
    </section>

    <section>
      <h3>IP Cameras (cycling every {{swap}}s)</h3>
...
// Soft carousel: every swap seconds, reload the <img> of one cam to ensure it stays fresh.
// (MJPEG is long-lived; reloading rotates attention and "cycles" underneath.)
let cur = 0;
setInterval(()=>{
  ...
  if(img){
    // Bounce the stream to refresh connection
