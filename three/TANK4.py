...
# -*- coding: utf-8 -*-
"""
TANK4 — Unified ROV Dashboard (USB robust + CSI Picamera2) with clean Ctrl-C shutdown
- Threaded HTTP server for stable MJPEG + control requests
- Starts in AUTO patrol (left↔right only: Y axis)
- Solid control UI: press & hold buttons, global keyboard, mobile touch
"""

import os, sys, time, signal, glob, math, io, threading
...
# Last command breadcrumb (for /health and UI)
last_cmd = {"when": 0.0, "cmd": None}
...
class SimpleCSICam:
    ...
    def frames(self):
        boundary = b"--frame\r\n"
        while not _stop.is_set() and not self._stop.is_set():
...
MOTORS = [
    ...
    {"ch": 8, "mid": 500, "nudge": 30, "pos": 400},  # Z (up/down)
]
CLAMP = (0, 4095)
...
class AutoConfig:
    ...
    tick_hz: float = 20.0
    enable_x: bool = False  # default OFF (we'll patrol Y only)
    enable_y: bool = True   # default ON  (left↔right)
    enable_z: bool = False
...
<head>
  ...
  <style>
    ...
    .pad { display:grid; grid-template-columns:repeat(3,60px); gap:8px; margin:6px 10px; }
    .pad button { width:60px; height:42px; }
...
<body>
  ...
  <main>
    <section class="hero">
      ...
      <div class="controls">
        <div class="pad">
          <button data-cmd="q">Up (Q)</button>
          <button data-cmd="w">Fwd (W)</button>
          <button data-cmd="e">Down (E)</button>
          <span></span>
          <button data-cmd="a">Left (A)</button>
          <button data-cmd="s">Back (S)</button>
          <button data-cmd="d">Right (D)</button>
        </div>
        <button id="stopBtn">Stop (Space/X)</button>
        <button id="autoStart">Auto: Start</button>
        <button id="autoStop">Auto: Stop</button>
      </div>
      <div class="toggles">
        <label><input type="checkbox" id="ax_x" onchange="pushAxes()"> Explore X (F/B)</label>
...
async function send(cmd){
  try {
    const r = await fetch('/cmd/'+encodeURIComponent(cmd), {method:'GET', cache:'no-store'});
    const t = await r.text();
    document.getElementById('status').textContent = t;
  } catch(e) {
    document.getElementById('status').textContent = 'ERR sending '+cmd+': '+e;
  }
}

// Press-and-hold support for mouse/touch on control pad
let holdTimer = null;
const HOLD_PERIOD_MS = 140; // how often to nudge while held
function holdStart(cmd){
  if(!cmd) return;
  clearInterval(holdTimer);
  send(cmd);
  holdTimer = setInterval(()=>send(cmd), HOLD_PERIOD_MS);
}
function holdEnd(){
  clearInterval(holdTimer);
}
...
function bindPad(){
  document.querySelectorAll('.pad button[data-cmd]').forEach(btn=>{
    ...
    btn.addEventListener('mousedown', ()=>holdStart(cmd));
    btn.addEventListener('mouseup', holdEnd);
    btn.addEventListener('mouseleave', holdEnd);
    btn.addEventListener('touchstart', (e)=>{ e.preventDefault(); holdStart(cmd); }, {passive:false});
    btn.addEventListener('touchend', (e)=>{ e.preventDefault(); holdEnd(); }, {passive:false});
  });
  document.getElementById('stopBtn').addEventListener('click', ()=>send('stop'));
  document.getElementById('autoStart').addEventListener('click', ()=>fetch('/auto/start').then(status));
  document.getElementById('autoStop').addEventListener('click', ()=>fetch('/auto/stop').then(status));
}

// Global key bindings; ensure focus
function bindKeys(){
  function handler(e, type){
    const k = (e.key||'').toLowerCase();
    if(['w','a','s','d','q','e'].includes(k)) { e.preventDefault(); send(k); }
    if(k===' ' || k==='x') { e.preventDefault(); send('stop'); }
  }
  window.addEventListener('keydown', (e)=>handler(e,'down'), {passive:false});
  document.body.addEventListener('click', ()=>{ window.focus(); }, {once:true});
}
...
async function status(){
  try{
    ...
    pwm.textContent = 'PWM: ' + (j.pwm_ok ? 'OK' : 'ERR');
...
async function loadUSBs(){
  try{
    const r = await fetch('/usbs', {cache:'no-store'}); const j = await r.json();
...
async function loadCSIs(){
  try{
    const r = await fetch('/csis', {cache:'no-store'}); const j = await r.json();
...
# USB MJPEG with Content-Length + no-cache
...
# CSI MJPEG with no-cache headers
def _csi_stream(idx: int):
    cam = csis.get(idx)
    if not cam:
        abort(404, f"CSI camera {idx} not running")
    headers = {"Cache-Control": "no-store, no-cache, must-revalidate, max-age=0", "Pragma": "no-cache"}
    return Response(cam.frames(), mimetype="multipart/x-mixed-replace; boundary=frame", headers=headers)
...
def cmd(c: str):
    ...
    if c == "stop":
        control.set_all_mid()
        last_cmd.update({"when": time.time(), "cmd": "stop"})
...
def auto_stop():
    control.set_mode("manual")
    control.set_all_mid()
    last_cmd.update({"when": time.time(), "cmd": "auto/stop"})
    return jsonify({"ok": True, "mode": control.mode})
...
def auto_axes():
    ...
    last_cmd.update({"when": time.time(), "cmd": f"axes:{kw}"})
...
def start_threads():
    ...
    # PWM watchdog + MotorController (start in AUTO, Y-only patrol)
    threading.Thread(target=pwm_watchdog, daemon=True).start()
    globals()["control"] = MotorController(MOTORS, pwm, pwm_lock)
    control.start()
    control.set_auto_params(enable_x=False, enable_y=True, enable_z=False)
    control.set_mode("auto")
...
def main():
    ...
    # Start a THREADED HTTP server so MJPEG streams don't block other requests.
    global _httpd, _http_thread
    try:
        _httpd = make_server(HOST, PORT, app, threaded=True)  # modern Werkzeug
    except TypeError:
        from werkzeug.serving import ThreadedWSGIServer, WSGIRequestHandler
        _httpd = ThreadedWSGIServer((HOST, PORT), WSGIRequestHandler)
        _httpd.set_app(app)
    try:
