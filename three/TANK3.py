...
# -*- coding: utf-8 -*-
"""
TANK3 — Unified ROV Dashboard (USB robust + CSI Picamera2) with clean Ctrl-C shutdown
- Uses werkzeug.make_server in a thread so we can shutdown() on SIGINT.
- Starts in AUTO patrol (left↔right only: Y axis).
"""

import os, sys, time, signal, glob, math, io, threading, shutil
...
def cleanup_and_exit(*_):
    ...
    try:
        ...
        pass
    # Hard exit if something is stuck
    os._exit(0)
...
def main():
    ...
    try:
        _httpd = make_server(HOST, PORT, app, threaded=True)  # works on modern Werkzeug
    except TypeError:
        # Fallback for older Werkzeug: build a threaded server manually
        from werkzeug.serving import ThreadedWSGIServer, WSGIRequestHandler
        _httpd = ThreadedWSGIServer((HOST, PORT), WSGIRequestHandler)
        _httpd.set_app(app)
    try:
        _httpd.daemon_threads = True  # let worker threads die when server stops
    except Exception:
        pass
    ...
    # Keep main thread alive until we’re told to stop
    while not _stop.is_set():
        time.sleep(0.2)
